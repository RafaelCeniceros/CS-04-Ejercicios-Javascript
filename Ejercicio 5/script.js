/* For each of the exercises below, assume you are starting with the following people array.
let people = ["Maria", "Dani", "Luis", "Juan", "Camila"];
1.	Write a command that prints out all of the people in the list.
2.	Write the command to remove "Dani" from the array.
3.	Write the command to remove "Juan" from the array.
4.	Write the command to move "Luis" to the front of the array.
5.	Write the command to add your name to the end of the array.
6.	Using a loop, iterate through this array and after console.log "Maria", exit from the loop. (usar break)
7.	Write the command that gives the indexOf where "Maria" is located.
At the end of the exercise, there should be 4 people in the array.
 */

let people = ["Maria", "Dani", "Luis", "Juan", "Camila"];

function printAllArray (arr){
  for(let i = 0; i< arr.length; i++){
    console.log(arr[i]);
  }
}

function removeFromArray(arr,element){
  //array.splice(index, howmany, item1, ....., itemX)
  arr.splice(arr.indexOf(element), 1);
}

function moveToFront(arr,element){
  removeFromArray(arr,element);
  arr.unshift(element);
}

function addToEnd(arr,element){
  arr.push(element);
}

function lookFor(arr,element){
  for(let i = 0; i< arr.length; i++){
    console.log(arr[i]);
    if(arr[i]==element){
      break;
    }
  }
}

function getIndex(arr,element)
{
  console.log(`Index of ${element}: is ${arr.indexOf(element)}`);
}

//Ejercicio 1
printAllArray(people);
//Ejercicio 2
removeFromArray(people,"Dani");
//Ejercicio 3
removeFromArray(people,"Juan");
//Ejercicio 4
moveToFront(people,"Luis");
//Ejercicio 5
addToEnd(people,"Rafa");
//Ejercicio 6
lookFor(people,"Maria");
//Ejercicio 7
getIndex(people,"Maria");